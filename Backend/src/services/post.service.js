...
const { isAdmin, isOwner } = require("../utils/role.util");
...
exports.getAllPosts = async (query, user) => {
  ...
  if (
    ...
    limitNumber < 1 ||
    limitNumber > MAX_POSTS_PER_PAGE
  ) {
    limitNumber = 10;
  }
  ...
  if (
    category &&
    category !== "all" &&
    !isValidCategoryByBoardType(board_type, category)
  ) {
...
// 커뮤니티 인기글 조회 (좋아요 10개 이상)
exports.getPopularPosts = async (query) => {
  const { category = "all", search = "", page = 1, limit = 15 } = query;
  ...
  const popularCategories = ["all", "free", "qna", "recruit"];
  if (!popularCategories.includes(category)) {
    const error = new Error("올바른 커뮤니티 카테고리가 아닙니다.");
    error.status = 400;
    throw error;
  }
  ...
  const popularPostIds = likeCounts
    .filter((item) => item._count.post_id >= 10)
  ...
  const where = {
    ...
    category: category === "all" ? { not: "notice" } : category,
    is_draft: false,
  };
  ...
  popularPosts.sort((a, b) => {
    const likeDifference = b._count.post_likes - a._count.post_likes;
  ...
  const totalCount = popularPosts.length;
  const startIndex = (pageNumber - 1) * limitNumber;

  return {
    posts: popularPosts.slice(startIndex, startIndex + limitNumber),
    pagination: {
      total: totalCount,
      page: pageNumber,
      limit: limitNumber,
...
exports.createPost = async ({ body, user }) => {
  ...
  if (!isValidBoardType(finalBoardType)) {
    const error = new Error(
      "올바른 게시판 타입이 아닙니다. (COMMUNITY, GALLERY, ARCHIVE, MAINTENANCE 중 하나)",
    );
    error.status = 400;
    throw error;
  }
...
exports.toggleDislike = async ({ postId, user }) => {
  ...
  if (existingLike) {
    const error = new Error(
      "이미 좋아요 누른 상태에서는 싫어요를 누를 수 없습니다.",
    );
    error.status = 400;
    throw error;
  }
...
exports.uploadPostImages = async (files) => {
  try {
    ...
    error.message =
      error.message || "게시글 이미지 업로드 중 오류가 발생했습니다.";
    throw error;
  }
};
...
exports.getMyPosts = async ({ user, query }) => {
  ...
  if (
    ...
    !validMyPostCategories.includes(normalizedCategory)
  ) {
    const error = new Error("올바른 카테고리가 아닙니다.");
    error.statusCode = 400;
    throw error;
